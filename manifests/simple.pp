...
class gluster::simple(
	...
	$baseport = '',	# specify base port option as used in glusterd.vol file
        ...
        $alternate_fqdn = '', # allows alternate fqdn's to be set (e.g. AWS)
	...
	class { '::gluster::server':
        ...
        if $alternate_fqdn != '' {
          $valid_fqdn = $alternate_fqdn
        }
        elsif "${::fqdn}" != '' {
          $valid_fqdn = "${::fqdn}"
        }
	else {
	  fail('Your $fqdn is empty and you have not provided an alternate. Please check your DNS settings.')
	}

	@@gluster::host { $valid_fqdn:
	}
	Gluster::Host <<||>>
	...
	if has_key($brick_params, $valid_fqdn) {
		$brick_params_list = $brick_params[$valid_fqdn]
	} else {
		$brick_params_list = []
	}
	...
	$brick_params_names = "${valid_count}" ? {
		...
		'1' => ["${valid_fqdn}:${valid_path}"],
		default => split(inline_template("<%= (1..@valid_count.to_i).collect{|i| '${valid_fqdn}:${valid_path}brick' + i.to_s.rjust(7, '0') + '/' }.join(',') %>"), ','),
	}
	...
	#@@gluster::brick { "${valid_fqdn}:${valid_path}":
	#	areyousure => true,
	#}
	Gluster::Brick <<||>>
